...
object MatrixRepo {
    ...
    private const val RECOVERY_ACCOUNT_DATA = "ai.tournesol.privacylodge.recovery"
    // Box config channel (feature B): box publishes status here; phone writes commands.
    private const val BOXSTATUS_TYPE = "ai.tournesol.privacylodge.boxstatus"
    private const val COMMAND_TYPE = "ai.tournesol.privacylodge.command"
    /** Box-published update state (feature H); read-only for the phone. */
    private const val UPDATE_TYPE = "ai.tournesol.privacylodge.update"
    /** Phone-written update preferences (feature H): {auto_check}. */
    private const val UPDATE_PREFS_TYPE = "ai.tournesol.privacylodge.update_prefs"
    private const val COMMAND_RESULT_TYPE = "ai.tournesol.privacylodge.command_result"
    private const val BACKUP_TYPE = "ai.tournesol.privacylodge.backup"
    ...
    private const val AGENTS_TYPE = "ai.tournesol.privacylodge.agents"
    ...
    suspend fun setAvatar(ctx: Context, uri: android.net.Uri) {
        ...
        val jpeg = ai.tournesol.privacybolt.util.ImageUtil.toAvatarJpeg(raw) ?: return
    ...
    suspend fun sendFile(ctx: Context, uri: android.net.Uri) {
        ...
        if (mime.startsWith("image/")) {
            ...
            val capped = raw?.let { ai.tournesol.privacybolt.util.ImageUtil.downscaleIfLarger(it) }
            if (capped != null) {
                runCatching { tmp.writeBytes(capped) }
                Log.i(TAG, "sent image capped to ${ai.tournesol.privacybolt.util.ImageUtil.SEND_MAX_PX}px (${raw.size}B → ${capped.size}B)")
            }
        }
    ...
    private const val AGENT_SESSIONS_TYPE = "ai.tournesol.privacylodge.agent_sessions"
    ...
    data class UpdateInfo(
        ...
        /** True when the box runs on an OS Privacy Bolt doesn't publish a box for (only Linux
    ...
    private const val BACKUP_ROOM_ACCOUNT_DATA = "ai.tournesol.privacylodge.backup_room"
    ...
    suspend fun ensureBackupRoom(): String? = backupRoomMutex.withLock {
        ...
        val rid = runCatching {
            c.createRoom(
                CreateRoomParameters(
                    name = "My files",
                    topic = "Privacy Bolt backup library",
